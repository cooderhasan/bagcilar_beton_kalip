...
export default function QuoteDetailModal({ request }: { request: QuoteRequest }) {
                ...
                <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-200">
                    <div className="p-6">
                        ...
                        <div className="flex items-start justify-between mb-6 border-b border-gray-100 pb-4">
                            <div>
                                <h3 className="text-xl font-bold text-gray-900">Teklif Detayı</h3>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="text-gray-900 font-medium mt-1">{request.name}</div>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Firma</label>
                                    <div className="text-gray-900 font-medium mt-1">{request.company || '-'}</div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="text-gray-900 mt-1 break-all">{request.email}</div>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Telefon</label>
                                    <div className="text-gray-900 mt-1 font-mono">{request.phone}</div>
                                </div>
                            </div>

                            {request.productInterest && (
                                <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                                    <label className="text-xs font-semibold text-amber-600 uppercase tracking-wider">İlgilenilen Ürün</label>
                                    <div className="text-amber-900 font-medium mt-1">{request.productInterest}</div>
                                </div>
                            )}
                            ...
                            {request.ipAddress && (
                                <div className="pt-4 border-t border-gray-100">
                                    ...
                                    <div className="text-gray-500 text-xs mt-1 font-mono">{request.ipAddress}</div>
                                </div>
                            )}
                        </div>
